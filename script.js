window.onload = function() {
	...
	// not actual twitchTV url
	xhr.open('GET', 'https://cors-anywhere.herokuapp.com', true)
  
	xhr.onload = function() {
		if(this.status == 200) {
			// hard-coded simulated api response
			var data = [
				{
					'stream': {
						'mature': false,
			...
			/**************************************
       *all streams shown by default
       *****************************************/
			for (var i = 0; i < data.length - 1; i++) {
				if (data[i]['stream'] == null) {
					output += '<div class="content"><img>' + data[i]['display_name'] + '</div>'
				} 
				else {
					output += '<div class="content"><img src="' + data[i]['stream'].logo 
          + '"><a href="' + data[i]['stream'].url 
          + '"target=_blank>' + data[i]['stream'].display_name 
          + '</a><p class="status">' + data[i]['stream'].status + '</p></div>'    
				}
			}

			var streamDisplay = document.getElementById('streams')
			streamDisplay.innerHTML = output
			...
			tabs.addEventListener('click', function(e) {
				...
				//  users online
				if (e.target.hash === '#online') {
					for (let i = 0; i < data.length - 1; i++) {
						if(data[i].stream !== null) {
							streamDisplay.innerHTML += '<div class="content"><img src="' + data[i]['stream'].logo 
              + '"><a href="' + data[i]['stream'].url 
              + '"target=_blank>' + data[i]['stream'].display_name 
              + '</a><p class="status">' + data[i]['stream'].status + '</p></div>' 
						}
				...
				//  users offline
				if (e.target.hash === '#offline') {
					for (let i = 0; i < data.length - 1; i++) {
						if(data[i].stream === null) {
							streamDisplay.innerHTML += '<div class="content"><img>' 
              + data[i]['display_name'] + '</div>'
						}
					}
				}        
			})
